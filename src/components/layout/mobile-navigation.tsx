...
import { headerCtaHref, primaryNavigation } from '@/config/navigation'
 ...
 * entry, full width after the links.
 */
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="mobile-sheet fixed inset-y-0 right-0 z-[var(--z-dialog)] flex w-full flex-col overflow-y-auto bg-surface-raised px-gutter pt-3 pb-6 shadow-dialog min-[30rem]:max-w-96 lg:hidden">
          ...
          <Dialog.Close asChild>
            <Link href={headerCtaHref(pathname)} className={cn(buttonVariants(), 'mt-6 w-full')}>
              {primaryCta.label}
            </Link>
          </Dialog.Close>
        </Dialog.Content>
