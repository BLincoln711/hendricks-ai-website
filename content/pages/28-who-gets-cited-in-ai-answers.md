...
**Description:** A research experiment. In one archived run, 247 distinct domains filled 308 citation slots across 20 AI answers, 86 percent of those domains were cited exactly once, and Perplexity and ChatGPT shared a single source.

**H1:** One Run Cited 247 Domains Across 308 Citation Slots, 86 Percent of Them Once. There Was No Incumbent to Displace.
...
## What one run of the citation set found
...
Every figure below is read from run 2026-08-19-110930, archived at history/runs/hendricks-2026-08-19-110930.json.

### 01. The citation set is nearly flat, so there is no incumbent to displace.
...
### 02. Perplexity and ChatGPT cited one domain in common, and the caveat matters.
...
### 03. The three engines contributed the set at completely different scales.

Perplexity supplied 233 distinct domains across 291 citation slots. ChatGPT supplied 11 domains across 11 slots. Google AI Overviews supplied 6 domains across 6 slots, on a run where 4 of its cells errored and returned no measurement. Almost the entire citation set in this run came from one engine. A figure drawn from this run and presented as what AI answers cite is describing Perplexity unless it says otherwise, and a reader handed any cross-engine figure should ask which engine produced it and how many slots the others contributed.

### 04. A community site and a professional network led the repeated citations.
...
### 05. A single AI visibility score averaged across engines measures incompatible things.
...
Read these before the tables. A domain, a citation slot, and an answer are three different units, and a concentration figure that quietly moves between them can make a flat set look like a contested one.
...
**Cell.** One question sent to one engine on one date. Seventeen questions across three engines produce 51 cells. A cell that errored returned no measurement and is counted separately from a cell that returned an answer citing nothing.

**Populated cell.** A measured cell in which the engine cited at least one source. Only populated cells contribute citation slots, so the populated count is the denominator for every figure about the structure of the citation set. This run had 20.

**Citation slot.** One distinct domain cited in one cell. A domain cited in six cells fills six slots. The slot count is the size of the citation set, and it is always at least the distinct-domain count.

**Distinct domain.** The number of different hosts appearing across all citation slots in a run. Counting is done at the host level, so every page on one site collapses into one domain.

**Singleton domain.** A domain that filled exactly one citation slot in the run. Read against the distinct-domain count it is the flatness measure: the higher the singleton share, the less any source recurs across answers.

**Concentration.** The share of citation slots held by the most-cited domains, published here as a count of slots against the total rather than as an index. A count can be checked against the tables on this page. An index cannot be checked against anything without the underlying data.

**Cross-engine overlap.** The number of domains appearing in the cited sets of two engines within the same run. It is read against the size of the smaller set, because an engine that filled few slots had few chances to overlap with anything.

## The citation set, in full

Eyebrow: The Data

One run, read five ways: the shape of the whole set, what each engine contributed, which domains recurred, which sat in the two-slot band, and what the two comparable engines had in common.

Table caption: The shape of the citation set in run 2026-08-19-110930.

| Measure | Run 2026-08-19-110930 |
| --- | --- |
| Cells sent | 51 |
| Cells measured | 47 |
| Cells that errored | 4, all Google AI Overviews |
| Populated cells, carrying at least one source | 20 |
| Citation slots filled | 308 |
| Distinct domains cited | 247 |
| Domains filling exactly one slot | 212, or 86 percent |
| Domains filling exactly two slots | 29, or 12 percent |
| Domains filling three slots | 2 |
| Slots held by the ten most-cited domains | 46 of 308, or 15 percent |

Table caption: Distinct domains and citation slots contributed by each engine, run 2026-08-19-110930.

| Engine | Distinct domains cited | Citation slots filled |
| --- | --- | --- |
| Perplexity | 233 | 291 |
| ChatGPT | 11 | 11 |
| Google AI Overviews | 6 | 6 |

Table caption: Domains that filled three or more citation slots in run 2026-08-19-110930.

| Domain | Citation slots filled |
| --- | --- |
| reddit.com | 12 |
| linkedin.com | 9 |
| semrush.com | 6 |
...
Table caption: Nine of the 29 domains that filled exactly two citation slots in run 2026-08-19-110930.
...
| probablygenius.com | 2 |
| growtika.com | 2 |
| airanklab.com | 2 |
| yotpo.com | 2 |
| searchengineland.com | 2 |

Table caption: Source overlap between Perplexity and ChatGPT, run 2026-08-19-110930.

| Measure | Run 2026-08-19-110930 |
| --- | --- |
| Distinct domains cited by Perplexity | 233 |
| Distinct domains cited by ChatGPT | 11 |
| Distinct domains across the two engines | 243 |
| Domains cited by both engines | 1, subscribepr.com |
...
### 02. Count cited URLs, not brand mentions.
...
### 03. Count slots first, then domains, and publish both.
...
### 04. Report concentration as counts against the total, never as an index.
...
### 05. Compare engines by what each contributed, never by an average across them.
...
### 06. Test cross-engine overlap on the raw domain sets, and publish the smaller set beside it.
...
### 07. Publish domains, and never a verdict about the firm behind one.
...
## Four things this study assumes

Eyebrow: Assumptions

### 01. That the host is the right unit to count a source by.
...
### 02. That the query set represents what a buyer in this category actually types.
...
### 03. That an API response resembles what a person sees.
...
### 04. That one reading describes one day and not a structure.
...
This is a description of one citation set. It is not an experiment in the sense that would let anyone claim a cause, and the difference is not a technicality. One run, one query set, one date, no intervention, no control, and no holdout. Nothing here tests whether any tactic produces a citation.
...
### 02. The cross-engine overlap rests on 11 ChatGPT slots.
...
### 03. Google AI Overviews contributed six slots, and four of its cells errored.
...
### 04. A flat set is not evidence that the set is easy to enter.
...
### 05. A domain absent from these 247 is not a domain absent from AI answers.
...
### 06. Nobody outside the platforms can observe why a source was chosen.
...
### 07. The set was read once, so nothing here measures stability.
...
- Two Runs in AI Answers. The same run of record, read for whether one brand appeared in it at all.
- What Is AI-Mediated Search? The surfaces this run sampled, and which of them Hendricks observes.
- The AI Selection Problem. What it means that an AI system recommended somebody else.
...
This measurement is applied in the Selection Intelligence solution, the Methodology, and the Search Intelligence Diagnostic.
...
**Primary CTA:** Establish a baseline through the Diagnostic

## Publication notes

Structured against the fifteen elements of the source-ready research format in `docs/06` section 12: direct answer, key findings, definitions, data, methodology, sample and date range, assumptions, limitations, sources, author, published date, updated date, data-through date, corrections link, related solution. The optional checked-evidence section is absent because this study ran no verification outside the probe.

No `FAQPage` JSON-LD, under any framing.

Research is not added to primary navigation. `content/pages/12-research.md` line 88 gates that on three category foundation pages, which is a different count from three published studies. Moving the gate is a separate decision.

This study reads the same run of record as `25-hendricks-selection-baseline.md`, 2026-08-19-110930, for a different question. That page asks whether one brand appeared in the citation set. This one describes the structure of the set. Neither restates the other's answer, and the two link to each other.
