...
**Description:** Brands are losing control over the path between being discovered and being chosen as AI systems interpret needs, compare options, and shape customer shortlists.

**H1:** Brands are losing control over the path between being discovered and being chosen.
...
`Need → Intent Interpretation → Research → Evaluation → Synthesis → Shortlist → Choice`
...
**Link:** [See the run these figures come from](/research/hendricks-selection-baseline)

The figures in this section are quoted from `/research/hendricks-selection-baseline` exactly as that page publishes them and nothing is derived from them. The per-engine counts and the populated-cell counts from the same run are published on `/what-is-ai-mediated-search` and are deliberately not repeated here.
...
Determine which needs and decisions matter.

### Understand selection

Observe whether the brand enters commercially relevant consideration.

### Engineer the presence

Improve the conditions the business can control.

### Prove impact

Measure what changes and state the evidence honestly.
...
This page argues that a brand can be absent from the options presented during an AI-assisted buying decision, and it carried no observation of its own. The link above is the dated instance, and it is the only entry in this page's related list recorded here. The rest of that list is an internal-linking decision under `docs/03` section 6 rather than approved copy, and it lives in `src/content/pages/ai-selection-problem.ts`.

The figures are quoted from `/research/hendricks-selection-baseline` exactly as that page publishes them. They describe two dated runs on two different query sets, with no intervention and no control. Nothing on this page may read them as a trend, as proof that a tactic works, or as a statement about brands in general.
...
**Applied in:** Selection Intelligence, the Diagnostic
