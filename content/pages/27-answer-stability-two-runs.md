...
**Description:** A research experiment. The same 17 questions, three AI systems, two runs seven hours apart. Whether an engine cited anyone matched on 51 of 51 cells. Which sources it cited had a mean overlap of 0.68.

**H1:** Two Runs, Seven Hours Apart, Agreed on Whether Each Answer Cited Anyone. They Disagreed on Which Sources.
...
## What one observation of an AI answer is worth
...
## Five findings from one run pair

Eyebrow: Key Findings

### 01. Whether an engine cited anyone at all did not move.
...
### 02. Which sources an engine cited moved on almost every cell that had sources.
...
### 03. The two halves carry different practical weight, and the weaker one is the one buyers look at.
...
### 04. The per-engine figures split three ways, and two of them rest on a handful of cells.
...
### 05. Nothing here explains why any source moved.
...
Read these before the tables. Two different comparisons run through this page, one coarse and one fine, and the whole result is that they behaved differently. A single stability score would flatten them into one number that describes neither.
...
**Query set.** The fixed list of buyer questions a run sends to every engine. Both runs on this page used the same 17 questions, in the same order, with nothing edited between them. A pair of runs on two different question sets measures the questions and cannot measure stability.

**Cell.** One question sent to one engine in one run. Seventeen questions across three engines produce 51 cells.

**Measured cell.** A cell where the engine returned an answer the probe could read. Run A measured 47 of its 51 cells. Run B measured 49.

**Errored cell.** A cell where the probe got no readable answer back. An errored cell is a broken instrument rather than a finding of absence. Run A errored on 4 cells, all of them Google AI Overviews. Run B errored on 2.

**Comparable cell.** A cell present in both runs, matched on the same question and the same engine. All 51 cells are comparable here, because both runs sent the same set to the same three engines.

**Citation state.** Whether a cell carried at least one citation. It takes two values, cited at least one source or cited none. This is the coarser of the two comparisons on this page and it is the one that held.

**Cited domain set.** The set of distinct hosts a cell cited in one run. Comparing the set from run A against the set from run B, for the same cell, is the finer of the two comparisons and it is the one that moved.

**Set overlap.** The size of the intersection of two cited domain sets divided by the size of their union, which is the Jaccard index. It reads 1.00 when the two runs cited exactly the same domains and 0 when they shared none. Every overlap figure on this page is that measure, reported as the comparison produced it.

**Identical set.** A cell whose cited domain set was exactly the same in both runs, an overlap of 1.00. Two of the 20 compared cells were identical.

**Run of record.** The archived result file a published figure is read from, named by a run id. Both runs here are archived and immutable: run A is 2026-08-19-110930 at history/runs/hendricks-2026-08-19-110930.json, and run B is 2026-08-19-181155 at history/runs/hendricks-2026-08-19-181155.json, each with a manifest beside it named for the same run id. A figure whose record cannot be produced on request is not a published measurement.

## Both runs, and both comparisons

Eyebrow: The Data

Two runs of the same 17 questions on the same three engines, roughly seven hours apart on 2026-08-19, with nothing changed in between. The first table is run health. The second and third are the comparison.

Table caption: Run health for the two 2026-08-19 runs, by measure.

| Measure | Run A, 2026-08-19-110930 | Run B, 2026-08-19-181155 |
| --- | --- | --- |
| Questions in the set | 17 | 17 |
| Engines queried | 3 | 3 |
| Cells run | 51 | 51 |
| Cells measured | 47 | 49 |
| Cells that errored | 4, all Google AI Overviews | 2 |
| Comparable cells | 51 | 51 |
| Archive file | hendricks-2026-08-19-110930.json | hendricks-2026-08-19-181155.json |

Table caption: What matched between the two runs, and what did not.

| Comparison | Cells compared | Result |
| --- | --- | --- |
| Whether the cell cited any source | 51 | 51 matched, 0 flipped in either direction |
| Which domains the cell cited | 20 | Mean overlap 0.68 |
| Cells returning an identical domain set | 20 | 2 |
| Cells returning no shared domain at all | 20 | 0 |
...
Table caption: Cited domain set overlap between the two runs, by engine.

| Engine | Cells compared | Identical sets | Mean overlap |
| --- | --- | --- | --- |
| Perplexity | 17 | 1 | 0.72 |
| ChatGPT | 2 | 0 | 0.22 |
| Google AI Overviews | 1 | 1 | 1.00 |
...
### 02. Change nothing else between the runs.
...
### 03. Run the pair on one date, hours apart.
...
### 04. Compare two things separately, and keep them separate.
...
### 05. Compare the domain sets with one named measure.
...
### 06. Report the run health next to the number.
...
### 07. Report the cell count behind every per-engine figure.
...
### 08. Archive both runs under their ids before publishing anything from them.
...
## Four things this comparison assumes

Eyebrow: Assumptions

### 01. That the query set represents what a buyer actually types.
...
### 02. That an API response resembles what a person sees.
...
### 03. That the cited domain set is the right unit to compare.
...
### 04. That seven hours is an interesting interval.
...
## What this run pair does not show

Eyebrow: Limitations

This is an observation of two archived runs. It is not an experiment. There is no intervention, no control, and no holdout, so nothing here attributes any change to any cause. The design varies elapsed time and nothing else.

### 01. There is no intervention and no control.
...
### 02. Two runs cannot establish a rate.
...
### 03. The ChatGPT and Google AI Overviews figures rest on 2 cells and 1 cell.
...
### 04. Nobody outside the platforms can observe why a source was chosen.
...
### 05. The result does not generalise past these conditions.
...
### 06. Errored cells were compared as cells that cited nothing.
...
### 07. The stable half is stable on this evidence and on nothing else.
...
### 08. This is not a Selection Stability measurement.
...
## Why this argues for repeated observation
...
- Two Runs in AI Answers: a Citation to a Deleted Page, Then Zero. The self-baseline study these two runs were taken alongside, with its own limits.
- Selection Intelligence. What a consideration and recommendation baseline covers, produces, and reports.
- What Is Selection Intelligence? The measures a baseline reports, defined before any of them is reported.
- Methodology. Context panels, classification, weighting, evidence grades, and the stated limits.
- What Is AI-Mediated Search? The surfaces these runs sampled, and which of them Hendricks observes.
...
Structured against the fifteen elements of the source-ready research format in `docs/06` section 12: direct answer, key findings, definitions, data, methodology, sample and date range, assumptions, limitations, sources, author, published date, updated date, data-through date, corrections link, related solution.

No `FAQPage` JSON-LD, under any framing.

No `errorsFound` section. The section is optional in `src/content/research/types.ts` and this run pair produced no verified defect in any answer. An empty section, or one carrying a defect from another study, would be furniture.

Every figure on this page is read from one of two archived runs, 2026-08-19-110930 and 2026-08-19-181155. Nothing is derived, averaged, rounded, or recomputed into a new statistic. The two run ids appear in the direct answer, the executive summary, the definitions, the data tables and their note, the methodology, the sample, the byline note, the corrections section, and the sources block, so a passage lifted out of this page carries its own provenance with it.
