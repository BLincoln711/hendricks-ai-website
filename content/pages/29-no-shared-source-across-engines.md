...
**Description:** A research experiment. In one archived run, 17 questions across three AI engines filled 449 citation slots with 319 distinct domains, no domain was cited by all three engines, and ChatGPT cited no source at all on 15 of the 17 questions it answered.

**H1:** Three Engines, 17 Questions, 449 Citations, and Not One Source Cited by All Three. ChatGPT Cited Nothing on 15 of 17.
...
This label is required and is not decorative. `CONTENT_VERIFICATION.md` holds `showResults` at false until two verified case studies exist, or one verified case study plus one clearly labeled research experiment. This page is a research experiment, `showResults` stays false, and `/results` stays dark.
...
## What one run of three engines found
...
## Five findings from one archived run

Every figure below is read from run 2026-08-20-110653, archived at history/runs/hendricks-2026-08-20-110653.json, except the six-run count in finding 02, which names its six runs in the sample section.

### 01. No domain was cited by all three engines.
...
### 02. ChatGPT cited a source on 2 of 17 questions, and the same 2 in each of six runs.
...
### 03. The 15 uncited answers were not empty. They were long.
...
### 04. Every engine cited a nearly flat set, which reproduces the earlier finding on a new run.
...
### 05. A blended cross-engine visibility score cannot describe a system a buyer can act on.
...
## The eight terms every number here depends on

Read these before the tables. The difference between a cell that returned no answer, a cell that answered and cited nothing, and a cell that cited a source is the difference between three findings on this page, and the category collapses all three into "visibility".

**Engine.** One AI answer system a run sends questions to. Three are covered here: Google AI Overviews, ChatGPT, and Perplexity. The scope statement under Limitations records what that set does and does not include.

**Cell.** One question sent to one engine on one date. Seventeen questions across three engines produce 51 cells. All 51 returned a measurement in this run.
...
**Populated cell.** A measured cell in which the engine cited at least one source. Only populated cells contribute citation slots. The gap between measured and populated is the whole of finding 02: ChatGPT measured 17 and populated 2.

**Citation slot.** One distinct domain cited in one cell. A domain cited in six cells fills six slots. The slot count is the size of the citation set and is always at least the distinct-domain count.

**Distinct domain.** The number of different hosts appearing across all citation slots. Counting is done at the host level, so every page on one site collapses into one domain.

**Cross-engine shared set.** The number of domains appearing in the cited sets of two engines within the same run, published beside the combined distinct count of the two engines so a reader can rebuild the ratio. It is read against the size of the smaller set, because an engine that filled few slots had few chances to overlap.

**Answer length.** The character count of the answer text an engine returned for one cell, recorded whether or not the answer cited anything. It is the measure that separates an engine that did not respond from an engine that responded without sources.

## Three engines, one question set, side by side

One run, read five ways: the shape of the whole set, what each engine contributed, what each pair had in common, how ChatGPT behaved across six consecutive runs, and how long its uncited answers were.

### The shape of the citation set in run 2026-08-20-110653, three engines combined.

| Measure | Run 2026-08-20-110653 |
|---|---|
| Questions | 17 |
| Engines | 3 |
| Cells sent | 51 |
| Cells measured | 51 |
| Cells that errored | 0 |
| Populated cells, carrying at least one source | 36 |
| Citation slots filled | 449 |
| Distinct domains cited | 319 |
| Domains filling exactly one slot | 252, or 79 percent |
| Slots held by the ten most-cited domains | 74 of 449, or 16 percent |
| Domains cited by all three engines | 0 |

### What each engine contributed in run 2026-08-20-110653.

| Engine | Cells measured | Cells citing a source | Citation slots | Distinct domains |
|---|---|---|---|---|
| Perplexity | 17 | 17 | 304 | 248 |
| Google AI Overviews | 17 | 17 | 136 | 110 |
| ChatGPT | 17 | 2 | 9 | 9 |

### Domains cited by both engines in each pair, run 2026-08-20-110653.

| Engine pair | Domains cited by both | Distinct domains across the pair |
|---|---|---|
| Perplexity and Google AI Overviews | 47 | 311 |
| ChatGPT and Perplexity | 1, subscribepr.com | 256 |
| ChatGPT and Google AI Overviews | 0 | 119 |
| All three engines | 0 | 319 |
...
### Cells citing at least one source, ChatGPT against Perplexity, six consecutive runs.

| Run id | ChatGPT cells citing a source | Perplexity cells citing a source |
|---|---|---|
| 2026-08-19-110930 | 2 of 17 | 17 of 17 |
| 2026-08-19-181155 | 2 of 17 | 17 of 17 |
| 2026-08-20-060002 | 2 of 17 | 17 of 17 |
| 2026-08-20-104059 | 2 of 17 | 17 of 17 |
| 2026-08-20-105338 | 2 of 17 | 17 of 17 |
| 2026-08-20-110653 | 2 of 17 | 17 of 17 |
...
### ChatGPT answer lengths in run 2026-08-20-110653, cited and uncited.

| Measure | ChatGPT, run 2026-08-20-110653 |
|---|---|
| Questions answered | 17 |
| Answers citing at least one source | 2 |
| Answers citing nothing | 15 |
| Shortest uncited answer | 2,136 characters |
| Longest uncited answer | 3,361 characters |
| Longest answer in the run, which did cite | 5,047 characters |
...
Both were found on 2026-08-20 while producing the run this page reports, and both had already generated a plausible finding that a person had written down before the defect surfaced. They are published because a firm selling measurement discipline does not get to show only the runs that worked.

### 01. A fourth engine failed every cell on an unsupported request field, and the failure looked like an answer.
...
### 02. One engine returned every citation behind a redirect, and the parser recorded the redirect as the source.
...
### 03. A finding drafted earlier the same day did not survive its own next run.
...
### 02. Count measured cells and populated cells separately, and publish both.
...
### 03. Record answer length whether or not the answer cited anything.
...
### 04. Count cited URLs at host level, not brand mentions.
...
### 05. Compare engines on raw domain sets, and publish the shared count with the combined count.
...
### 06. Never publish an overlap without the size of the smaller set.
...
### 07. Take the cross-run count from archived runs, never from a live re-query.
...
### 08. Report the run health next to the number.
...
CTA: Read the full Hendricks methodology
...
### 01. That a shared run makes three engines comparable.
...
### 02. That the host is the right unit to count a source by.
...
### 03. That the query set represents what a buyer in this category actually types.
...
### 04. That an API response resembles what a person sees.
...
### 05. That six runs in 26 hours describe a window and not a permanent behaviour.
...
This is a description of three citation sets taken at one time. It is not an experiment in the sense that would let anyone claim a cause, and the difference is not a technicality. One run, one query set, one date, no intervention, no control, no holdout. Nothing here tests whether any tactic produces a citation.

### 01. One run, one query set, one date, one geography, one language.
...
### 02. Both ChatGPT overlap figures rest on 9 citation slots.
...
### 03. The zero shared across all three is partly a consequence of that same small set.
...
### 04. Why an engine retrieved or did not is not observable from outside.
...
### 05. Google AI Overviews populated-cell counts varied widely across the six runs.
...
### 06. A flat set is not evidence that the set is easy to enter.
...
### 07. A domain absent from these 319 is not a domain absent from AI answers.
...
### 08. This is not a Selection Intelligence baseline.
...
**Author:** Brandon Lincoln Hendricks, Search Intelligence Engineer, Hendricks
...
CTA: See what a Selection Intelligence baseline covers

CTA: Read the Methodology

## Related

- Who Gets Cited in AI Answers. The structure of a citation set on an earlier run, and why there was no incumbent in it.
- Two Runs, Same Questions. How much a single reading of an AI answer can be trusted when the question is asked twice.
- The Hendricks Selection Baseline. The same instrument, read for whether one brand appeared in the answers at all.
- What Is AI-Mediated Search? The surfaces this run sampled, and which of them Hendricks observes.
- Do You Need an AI Visibility Tool or a Partner? What a monitoring subscription produces, and which jobs it leaves to a person.
...
The article module is `src/content/research/no-shared-source-across-engines.ts` and the registry entry is in `src/content/research/index.ts`. The route is `researchNoSharedSourceAcrossEngines` in `src/config/routes.ts`.
